...
void main (void) {
	...
	// sweep
	color += sweep(v_uv, vec2(0.5), 0.3, 0.002, edge_thickness) * vec3(0.0, 1.0, 1.0); 

    // polygons
    float p1 = 0.9 - sin(u_time * 3.0) * 0.05;
    float p2 = 0.1 - sin(u_time * 3.0 + PI) * 0.05;
    color += polygon(v_uv, vec2(p1, 0.5), 0.005, 3, 0.0, 0.001);
    color += polygon(v_uv, vec2(p2, 0.5), 0.005, 3, PI, 0.001);
    color += polygon(v_uv, vec2(0.5, p1), 0.005, 3, 0.5, 0.001);
    color += polygon(v_uv, vec2(0.5, p2), 0.005, 3, PI * 0.5, 0.001);
